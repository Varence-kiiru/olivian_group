...
class GroupSlug(models.Model):
    """Stores a normalized slug for Django auth Groups to ensure consistent matching."""

    from django.contrib.auth.models import Group as AuthGroup

    group = models.OneToOneField('auth.Group', on_delete=models.CASCADE, related_name='chat_slug')
    ...
    def __str__(self):
        return f"{self.group.name} -> {self.slug}"


class FavoriteRoom(models.Model):
    """User favorite/pinned chat rooms"""

    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='favorite_rooms')
    room = models.ForeignKey('ChatRoom', on_delete=models.CASCADE, related_name='favorited_by')
...
class RoomMute(models.Model):
    """Per-user mute setting for a chat room"""
    ...
    def __str__(self):
        return f"{self.user.username} muted {self.room.name}"
