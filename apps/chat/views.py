...
from .models import ChatRoom, Message, MessageReadStatus, NotificationPreference, UserActivity, MessageReaction, GroupSlug, PushSubscription, RoomMute
...
def chat_dashboard(request):
    ...
    for room in rooms:
        if room.room_type == 'private':
            ...
            # Check if user belongs to appropriate department groups using exact slug match
            room_slug = slugify(room.name)
            user_group_slugs = get_user_group_slugs(user)
            if room_slug in user_group_slugs or user.is_staff:
                accessible_rooms.append(room)
        elif room.room_type == 'general':
            # General rooms are accessible to all authenticated users
            accessible_rooms.append(room)
        elif room.room_type == 'project':
            # Check project access (management/staff groups or exact slug match)
            room_slug = slugify(room.name)
            user_group_slugs = get_user_group_slugs(user)
            if user.groups.filter(name__in=['management', 'staff']).exists() or room_slug in user_group_slugs or user.is_staff:
                accessible_rooms.append(room)
        else:
            accessible_rooms.append(room)
    ...
    # Mark favorites on rooms for current user
    fav_ids = set(FavoriteRoom.objects.filter(user=user).values_list('room_id', flat=True))
    for room in accessible_rooms:
        room.is_favorited = room.id in fav_ids
...
def chat_room(request, room_name):
    ...
    if room.room_type == 'private':
        can_access = room.participants.filter(id=user.id).exists()
            ...
            # Exact slug match for department group names
            room_slug = slugify(room.name)
            user_group_slugs = get_user_group_slugs(user)
            can_access = room_slug in user_group_slugs or user.is_staff
    elif room.room_type == 'general':
        # General rooms are accessible to all authenticated users
        can_access = True
    elif room.room_type == 'project':
        # Project access via management/staff groups or exact slug match
        room_slug = slugify(room.name)
        user_group_slugs = {slugify(g.name) for g in user.groups.all()}
        can_access = user.groups.filter(name__in=['management', 'staff']).exists() or room_slug in user_group_slugs or user.is_staff
    else:
        can_access = True
    ...
    if room.room_type == 'private':
        ...
        room_slug = slugify(room.name)
        user_group_slugs = {slugify(g.name) for g in user.groups.all()}
        online_users = [activity.user for activity in online_activities if (room_slug in user_group_slugs) or user.is_staff or activity.user.id == user.id]
    elif room.room_type == 'general':
        online_users = [activity.user for activity in online_activities]
    elif room.room_type == 'project':
        room_slug = slugify(room.name)
        user_group_slugs = {slugify(g.name) for g in user.groups.all()}
        online_users = [activity.user for activity in online_activities if user.groups.filter(name__in=['management', 'staff']).exists() or room_slug in user_group_slugs or user.is_staff or activity.user.id == user.id]
    else:
        online_users = [activity.user for activity in online_activities]
...
def room_members(request, room_name):
    ...
    invitation_form = RoomInvitationForm(room=room)  # Initialize form
    if request.method == 'POST' and can_manage:
        action = request.POST.get('action')

        if action == 'invite' and room.room_type in ['private', 'project']:
            invitation_form = RoomInvitationForm(request.POST, room=room)
            if invitation_form.is_valid():
                users_to_add = invitation_form.cleaned_data['usernames']
    ...
    if room.room_type == 'private':
        ...
        room_slug = slugify(room.name)
        group_names = [g.name for g in Group.objects.all() if slugify(g.name) == room_slug]
        members = []
        ...
        # For project rooms, show participants
        members = room.participants.all()
    else:
        members = []
...
@login_required
def api_toggle_favorite(request, room_name):
    """Toggle favorite/pin for a room for the current user"""
    if request.method != 'POST':
    ...
    # Check access quickly
    can_access = True
    if room.room_type == 'private' and not room.participants.filter(id=user.id).exists():
        can_access = False
    if not can_access:
    ...
    fav = None
    try:
        fav = FavoriteRoom.objects.filter(user=user, room=room).first()
    except Exception:
        fav = None
...
def api_rooms_list(request):
    """Paginated list of accessible rooms, with favorite flag. Query: page, limit, filter (department/project/general/private/all)"""
    user = request.user
    page = int(request.GET.get('page', 1))
    limit = int(request.GET.get('limit', 25))
    limit = min(max(limit, 5), 100)
    room_filter = request.GET.get('filter', 'all')

    rooms_qs = ChatRoom.objects.filter(is_active=True)

    # Filter by type
    if room_filter in ['department', 'project', 'general', 'private']:
        rooms_qs = rooms_qs.filter(room_type=room_filter)

    accessible = []
    for room in rooms_qs.order_by('-created_at'):
        can_access = False
        if room.room_type == 'private':
            can_access = room.participants.filter(id=user.id).exists()
        elif room.room_type == 'department':
    ...
    # Attach favorite flags and last message
    fav_room_ids = set(FavoriteRoom.objects.filter(user=user).values_list('room_id', flat=True))

    # Sort: favorites first by pinned_at, then recent activity (last_message timestamp), then created_at
    def room_key(r):
        fav = (1 if r.id in fav_room_ids else 0)
        last_msg = Message.objects.filter(room=r).order_by('-timestamp').values_list('timestamp', flat=True).first()
        last_ts = last_msg.timestamp() if last_msg else 0
        return (fav, last_ts)

    # simple slicing for pagination
    start = (page - 1) * limit
    end = start + limit
    sliced = accessible[start:end]
    ...
    for room in sliced:
        ...
        payload.append({
            'name': room.name,
            'display_name': room.name.title(),
            'type': room.room_type,
            'description': room.description or '',
            'favorited': room.id in fav_room_ids,
            'last_message': {
                'author': last_msg.author.username if last_msg else None,
                'content': (last_msg.content[:120] + '...') if last_msg and len(last_msg.content) > 120 else (last_msg.content if last_msg else ''),
                'timestamp': last_msg.timestamp.isoformat() if last_msg else None,
            } if last_msg else None,
        })

    has_more = len(accessible) > end

    return JsonResponse({'success': True, 'rooms': payload, 'page': page, 'limit': limit, 'has_more': has_more})
...
def api_subscribe_push(request):
    ...
    Expects JSON body with fields: endpoint, keys: {p256dh, auth}
    """
    import json
    try:
